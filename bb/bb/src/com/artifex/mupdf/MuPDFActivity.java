...
public class MuPDFActivity extends Activity
{
	...
	public void createUI(Bundle savedInstanceState)
	{
		...
		mDocView = new ReaderView(this)
		{
			...
			{
				if (SearchTaskResult.get() != null && SearchTaskResult.get().pageNumber == i) ((PageView) v).setSearchBoxes(SearchTaskResult.get().searchBoxes);
				else
		...
		setContentView(R.layout.viewer_header_bar);
		...
		LinearLayout lyPdfViwer 			= (LinearLayout) findViewById(R.id.lyPdfViwer);
		...
		mSearchTask = new SafeAsyncTask<Void, Integer, SearchTaskResult>()
		{
			...
			protected SearchTaskResult doInBackground(Void... params)
			{
				...
				while (0 <= index && index < core.countPages() && !isCancelled())
				{
					...
					if (searchHits != null && searchHits.length > 0) return new SearchTaskResult(mSearchText.getText().toString(), index, searchHits);
